...
static float read_onboard_temperature(bool centigrade) {
...
static void publish_temperature(MQTT_CLIENT_DATA_T *state) {
    ...
    float temperature = read_onboard_temperature(degC);
...
static void sub_unsub_topics(MQTT_CLIENT_DATA_T* state, bool sub) {
    ...
    mqtt_sub_unsub(state->mqtt_client_inst, full_topic(state, "/units"), MQTT_SUBSCRIBE_QOS, cb, state, sub);
}
...
static void mqtt_incoming_data_cb(void *arg, const u8_t *data, u16_t len, u8_t flags) {
    ...
    if (strcmp(basic_topic, "/led") == 0)
    {
        ...
        INFO_printf("%.*s\n", len, data);
    } else if (strcmp(basic_topic, "/units") == 0) {
        degC = strcmp((const char *)state->data, "1") == 0;
